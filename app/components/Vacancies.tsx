  ...
  if (loading) {
    return (
      <div className="text-center py-16 text-[#C9A86A] text-lg font-medium">
        Conectando con las mejores oportunidades de desarrollo...
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="text-center mb-12 space-y-3">
        <span className="inline-block px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-[#0A1A3A] bg-[#C9A86A] rounded-full shadow-md">
        ...
        <p className="text-gray-300 text-sm sm:text-base max-w-2xl mx-auto font-light leading-relaxed">
          Conectamos a profesionales extraordinarios con organizaciones que impulsan su desarrollo. Tu próximo paso profesional comienza aquí.
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-2xl mb-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-center text-gray-900 max-w-4xl mx-auto">
        <div>
          ...
          <input
            type="text"
            placeholder="Ej. Ingeniería, Ventas, Logística..."
            value={searchTerm}
      ...
      {filteredVacancies.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredVacancies.map((v) => (
            <div
              key={v.id}
              className="p-8 rounded-2xl bg-white text-gray-900 border-t-4 border-[#C9A86A] shadow-2xl flex flex-col justify-between w-full min-h-[440px] transition-all hover:-translate-y-1 hover:shadow-[#C9A86A]/20 duration-200"
            >
              <div className="space-y-4">
                <span className="text-[11px] font-bold text-[#8c6f33] uppercase tracking-widest block">
                ...
                <div className="text-xs font-semibold flex flex-wrap gap-2 pt-1">
                  ...
                  <span className="bg-[#C9A86A]/15 px-3 py-1.5 rounded-lg border border-[#C9A86A]/30 text-[#8c6f33]">
                    <span className="font-bold mr-1">Ofrecimiento:</span>{" "}
              ...
              <Link
                ...
                className="w-full py-3.5 bg-[#0A1A3A] text-[#C9A86A] border border-[#C9A86A]/30 font-bold rounded-xl hover:bg-[#122b5c] transition text-sm shadow-lg mt-6 text-center block"
              >
