...
export default function Acceso() {
  ...
  const handleLogin = async () => {
    ...
    // Redirigir a la página de Actualización de Datos con el ID del candidato
    router.push(`/candidatos/actualizar/${data.id}`);
  };
  ...
  return (
    <section className="min-h-screen bg-[#0A1A3A] text-white flex items-center justify-center px-6">
      <div className="bg-white text-[#0A1A3A] p-10 rounded-xl shadow-xl w-full max-w-md">
        ...
        <input
          type="password"
          className="border p-3 rounded w-full mb-6 text-gray-900"
          value={password}
        ...
        <button
          onClick={handleLogin}
          className="w-full py-3 bg-[#C9A86A] text-[#0A1A3A] font-bold rounded-md hover:bg-[#d8b97a] transition shadow-md"
        >
          Entrar
        </button>
      </div>
